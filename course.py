...
user = [
    ['Dzulfikar Sadid','dzulfikar@mail.com','08123456789'],
...
def sort_and_refresh(lst, compare, refresh):
    global is_acending
    if compare == compare_kelas_ascending or compare == compare_user_ascending:
        is_acending = True
    else:
        is_acending = False
...
# Searching
def binary_search(lst, target, compare):
    left = 0
    right = len(lst) - 1
    while left <= right:
        mid = (left + right) // 2
        print(lst[mid][0])
        if compare(lst[mid][0], target):
            left = mid + 1
        elif compare(target, lst[mid][0]):
            right = mid - 1
        else:
            return mid
...
def search_kelas(query):
    if is_acending:
        ...
        result = binary_search(kelas, query,compare_kelas_descending)
    else:
        bubble_sort(kelas, compare_kelas_descending)
        result = binary_search(kelas, query,compare_kelas_ascending)
    if result >= 0:
...
def search_user(query):
    if is_acending:
        ...
        result = binary_search(user, query,compare_user_descending)
    else:
        bubble_sort(user, compare_user_descending)
        result = binary_search(user, query,compare_user_ascending)
    if result >= 0:
...
def refresh_kelas():
    ...
    for i in range(len(kelas)):
        ...
        join_button = tk.Button(kelas_frame, text="Join Class", font=("Arial", 16), relief="groove", padx=5, pady=2, width=10, borderwidth=2, bd=1, bg="blue", fg="white", highlightthickness=0,  highlightcolor="white", command=lambda: show_join_class_form(kelas[i]))
...
def refresh_user_kelas():
    ...
    for i in range(len(kelas)):
        ...
        join_button = tk.Button(kelas_frame, text="Show Users", font=("Arial", 16), relief="groove", padx=5, pady=2, width=10, borderwidth=2, bd=1, bg="yellow", fg="black", highlightthickness=0,  highlightcolor="white", command=lambda kelas=kelas[i][0]: show_user_kelas(kelas))
        join_button.pack(side="left", pady=5)
...
def show_join_class_form(kelas):
    ...
    submit_button = tk.Button(form_window, text="Submit", command=lambda: submit_form(name_entry.get(), email_entry.get(),phone_entry.get(), kelas[0]))
...
def show_kelas_search(index,kelas_detail):
    messagebox.showinfo(f"Kelas Ditemukan di Posisi {index}", f"Nama Kelas: {kelas_detail[0]}\nMentor: {kelas_detail[1]}\nDurasi: {kelas_detail[2]} bulan")
...
def show_user_search(index,user_detail):
    messagebox.showinfo(f"User Ditemukan di Posisi {index}", f"Nama: {user_detail[0]}\nEmail: {user_detail[1]}\nNomor: {user_detail[2]}")
...
def show_user_kelas(kelas):
    ...
    user_list = "\n".join(["{}. {}".format(i+1, user) for i, user in enumerate(filtered_users)])
    if len(filtered_users) == 0:
        messagebox.showinfo("Filtered Users", "Tidak ada user yang terdaftar di kelas ini")
    else:    
...
# Memanggil fungsi
sort_and_refresh(kelas, compare_kelas_ascending, refresh_kelas)
sort_and_refresh(user, compare_user_ascending, refresh_user)
tampilkan_menu_gui()